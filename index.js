...
// ----- Parse the styles document and add the styles to the respective products
...
MongoClient.connect(url, function (err, client) {
  ...
  Papa.parse(styles, { delimiter: ",", header: true, step: (results) => {
...
//           let thisPromise = new Promise((resolve, reject) => {
//             db.collection(workingCollection).updateOne({ styles: {$elemMatch: { id: data.styleId }}}, {$push: {"styles.$.skus" : data }})
//             .then((result) => {
//               resolve(result);
//             })
//             .catch((error) => {
